...
`app.bind_session` returns `{ binding_id, project_id, revision_id }`. `app.get_projection`/`projection` returns data `{ binding_id, project, context, search_spec, search_results, proposals }`, where project is a nullable existing ProjectSnapshot. Event kinds include `project_changed`, `revision_advanced`, `context_changed`, `search_changed`, `navigation_requested`, `reveal_requested`, `proposal_changed`, `operation_changed`. Navigation payload `{ tab, request_id }`; reveal payload `{ segment_id?, alignment_id?, node_id?, request_id }`. Native event channel command `agent_subscribe`, call command `agent_call` (argument `{ call }`), projection command `agent_projection`. Subscribe before loading projection, discard <= snapshot sequence, resnapshot on gaps. Numbers crossing JS for sequence/revision are decimal strings.
