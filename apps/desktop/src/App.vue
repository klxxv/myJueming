...
const parallelWorkspaceRef = ref<InstanceType<typeof ParallelWorkspace> | null>(null);
...
const mergeAlignments = async (alignmentIds: string[]) => {
  if (!requireOpenProject()) return;
  busy.value = true;
  try { await applySnapshot(await kernelClient.mergeAlignments(alignmentIds)); clearWorkspaceSelection(); notify(`已合并 ${alignmentIds.length} 个 Alignment`); }
...
const splitAlignment = async (alignmentId: string, sourceGroups: string[][], targetGroups: string[][]) => {
  if (!requireOpenProject()) return;
  busy.value = true;
  try { await applySnapshot(await kernelClient.splitAlignment(alignmentId, sourceGroups, targetGroups)); clearWorkspaceSelection(); notify("已拆分 Alignment"); }
