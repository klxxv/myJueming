...
import { createKernelClient, makeImportProfile, snapshotToWorkspace, type AlignmentDto, type BookmarkPreviewDto, type Encoding, type ExportFormat, type ImportPreviewResponse, type ProjectSnapshot, type ProjectSummaryDto, type ReplacePreviewRequest, type RevisionComparison, type SegmentationMode, type SegmentDto, type SupportedLanguageDto, type WorkspaceMode } from "./domain/kernel-client";
...
type ShortcutProfile = "auto" | "macos" | "windows";
...
const ltrLanguages = computed(() => supportedLanguages.value.map((language) => ({ id: language.language_id, label: language.native_name })));
...
const usesMacShortcuts = computed(() => shortcutProfile.value === "macos" || (shortcutProfile.value === "auto" && detectedMacOS));
const shortcutLabels = computed(() => usesMacShortcuts.value ? {
  newProject: "⌘N", openProject: "⌘O", save: "⌘S", find: "⌘F", nextFind: "⌘G", previousFind: "⇧⌘G", undo: "⌘Z", redo: "⇧⌘Z", commitEdit: "⌘↵",
} : {
  newProject: "Ctrl+N", openProject: "Ctrl+O", save: "Ctrl+S", find: "Ctrl+F", nextFind: "Ctrl+G", previousFind: "Ctrl+Shift+G", undo: "Ctrl+Z", redo: "Ctrl+Y", commitEdit: "Ctrl+Enter",
});
...
onMounted(async () => {
  ...
  const savedCleanupPolicy = localStorage.getItem("jueming-cache-cleanup-policy");
  const savedCleanupAt = Number(localStorage.getItem("jueming-cache-cleanup-at"));
  const savedShortcutProfile = localStorage.getItem("jueming-shortcut-profile");
  ...
  if ([1000, 3000, 5000, 10000, 30000].includes(savedAutosaveDelay)) autoSaveDelayMs.value = savedAutosaveDelay;
  if (["startup", "weekly", "monthly", "never"].includes(savedCleanupPolicy ?? "")) cacheCleanupPolicy.value = savedCleanupPolicy as typeof cacheCleanupPolicy.value;
  if (Number.isFinite(savedCleanupAt) && savedCleanupAt > 0) lastCacheCleanupAt.value = savedCleanupAt;
  if (["auto", "macos", "windows"].includes(savedShortcutProfile ?? "")) shortcutProfile.value = savedShortcutProfile as ShortcutProfile;
  if (savedTrackpadOptimized === "true" || savedTrackpadOptimized === "false") trackpadOptimized.value = savedTrackpadOptimized === "true";
  applyUiSettings();
  applyInteractionSettings(false);
  try { supportedLanguages.value = await kernelClient.listSupportedLanguages(); } catch { supportedLanguages.value = fallbackLanguages; }
  try {
...
const setMode = (mode: WorkspaceMode) => {
  ...
  if (mode === "order" && activeMode.value !== "order") orderBaseline.value = sourceRows.value.map((segment) => segment.id);
...
const resetImport = () => { sourcePath.value = ""; targetPath.value = ""; projectDirectory.value = ""; sourceLanguage.value = "zh"; targetLanguage.value = "en"; sourcePreview.value = null; targetPreview.value = null; previewTab.value = "source"; };
...
const createProjectFromPreview = async () => {
  ...
  try {
    const snapshot = await kernelClient.createProject({
      ...
      source: { language_id: sourceLanguage.value, title: `${displayLanguageLabel(sourceLanguage.value)}（原文）`, input: { kind: "file", path: sourcePath.value }, profile: makeImportProfile(sourceEncoding.value, sourceSegmentation.value) },
      target: { language_id: targetLanguage.value, title: `${displayLanguageLabel(targetLanguage.value)}（译文）`, input: { kind: "file", path: targetPath.value }, profile: makeImportProfile(targetEncoding.value, targetSegmentation.value) },
    });
    const createdProjectPath = projectPath();
...
const reorderSegment = async (orderedSegmentIds: string[]) => {
  if (!requireOpenProject()) return;
  busy.value = true;
  try { await applySnapshot(await kernelClient.reorderSegments(orderedSegmentIds)); clearWorkspaceSelection(); notify("拖拽排序已自动保存，Alignment 关系保持稳定"); }
...
const resetOrder = async () => {
  ...
  busy.value = true;
  try { await applySnapshot(await kernelClient.reorderSegments(orderBaseline.value)); clearWorkspaceSelection(); notify("已恢复进入顺序模式时的句段顺序"); }
...
function applyInteractionSettings(notifyUser: boolean | Event = true) {
  document.documentElement.dataset.platform = detectedMacOS ? "macos" : "windows";
  ...
  if (notifyUser !== false) notify(`快捷键已切换为${usesMacShortcuts.value ? "macOS" : "Windows / Linux"}布局`);
}
...
async function clearProjectCache(manual = true) {
  ...
  try {
    const removedBytes = await kernelClient.clearCache();
    lastCacheCleanupAt.value = Date.now();
    localStorage.setItem("jueming-cache-cleanup-at", String(lastCacheCleanupAt.value));
    if (manual) notify(`已清理 ${(removedBytes / 1024).toFixed(1)} KB 可重建缓存；Revision 历史未删除`);
  } catch (error) { if (manual) notify(`缓存清理失败：${errorMessage(error)}`); }
...
async function maybeCleanupCache() {
  ...
  const due = cacheCleanupPolicy.value === "startup" || (cacheCleanupPolicy.value === "weekly" && elapsed >= 7 * 86400000) || (cacheCleanupPolicy.value === "monthly" && elapsed >= 30 * 86400000);
...
const handleShortcut = (event: KeyboardEvent) => {
  ...
  if (key === "n" && !event.shiftKey) { event.preventDefault(); void openImport("new"); return; }
  if (key === "o" && !event.shiftKey) { event.preventDefault(); void openImport("open"); return; }
  ...
  <div class="app-shell" :class="{ 'app-shell--macos': detectedMacOS }">
    ...
    <div class="content-grid" :class="{ 'content-grid--collapsed': sideNavCollapsed, 'content-grid--annotation': annotationOpen }">
      ...
      <main class="main-stage">
        ...
        <section v-else-if="activeNav === 'settings'" class="aux-view settings-view">
          <Settings2 :size="28" /><h2>设置</h2><p>工程内容完全离线保存；自动保存提交完整 Revision，缓存清理不删除正式历史。</p>
          <div class="settings-card">
            <h3>外观</h3>
            <label>显示主题<select v-model="theme" @change="applyUiSettings"><option value="light">明亮</option><option value="eye">护眼</option></select></label>
            <label>界面缩放 <input v-model.number="fontScale" type="range" min="85" max="130" step="5" @change="applyUiSettings" /><output>{{ fontScale }}%</output></label>
            <h3>macOS 与快捷键</h3>
            <label>快捷键布局<select v-model="shortcutProfile" @change="applyInteractionSettings"><option value="auto">自动识别（推荐）</option><option value="macos">macOS · Command</option><option value="windows">Windows / Linux · Ctrl</option></select><output>{{ usesMacShortcuts ? '⌘' : 'Ctrl' }}</output></label>
            <label class="toggle-setting">触控板优化<input v-model="trackpadOptimized" type="checkbox" @change="applyInteractionSettings" /><span>{{ trackpadOptimized ? '已开启' : '已关闭' }}</span></label>
            ...
            <h3>自动保存与历史</h3>
            <label>保存延迟<select v-model.number="autoSaveDelayMs" @change="applyPersistenceSettings"><option :value="1000">1 秒</option><option :value="3000">3 秒（推荐）</option><option :value="5000">5 秒</option><option :value="10000">10 秒</option><option :value="30000">30 秒</option></select><output>{{ autoSaveDelayMs / 1000 }}s</output></label>
            <small>停止输入达到该时间后，将当前句段作为一个完整 ChangeSet 写入 Revision 历史。</small>
            <label>缓存清理<select v-model="cacheCleanupPolicy" @change="applyPersistenceSettings"><option value="startup">每次启动</option><option value="weekly">每 7 天（推荐）</option><option value="monthly">每 30 天</option><option value="never">从不自动清理</option></select></label>
            <div class="cache-actions"><button class="secondary-button" type="button" :disabled="cacheCleaning" @click="clearProjectCache(true)">{{ cacheCleaning ? '清理中…' : '立即清理派生缓存' }}</button><span v-if="lastCacheCleanupAt">上次：{{ new Date(lastCacheCleanupAt).toLocaleString('zh-CN', { hour12: false }) }}</span></div>
    ...
    <div v-if="importOpen" class="modal-backdrop" @click.self="importOpen = false"><section class="import-modal" role="dialog" aria-modal="true" aria-labelledby="import-title"><header><div><span class="eyebrow">NEW PROJECT</span><h2 id="import-title">新建平行工程</h2></div><button type="button" title="关闭导入向导" @click="importOpen = false"><X :size="19" /></button></header><div class="import-project-grid"><label>项目名称<input v-model="projectName" placeholder="例如：阿古顿巴_中英对齐" /></label><label>保存位置<div class="compact-picker"><input v-model="projectDirectory" readonly placeholder="选择父文件夹" /><button type="button" @click="chooseProjectDirectory">浏览…</button></div></label></div><div class="language-pair-picker"><label>原文语言 <select v-model="sourceLanguage"><option v-for="language in ltrLanguages" :key="language.id" :value="language.id">{{ language.label }}</option></select></label><label>译文语言 <select v-model="targetLanguage"><option v-for="language in ltrLanguages" :key="language.id" :value="language.id">{{ language.label }}</option></select></label><small>支持最常用的十种从左到右书写语言；语言只标注文档，不自动翻译或改变文本。</small></div><div class="import-source-grid"><section><div class="import-path"><label for="source-path">{{ displayLanguageLabel(sourceLanguage) }}（原文）</label><div><FolderOpen :size="17" /><input id="source-path" v-model="sourcePath" readonly placeholder="选择 TXT 原文" /><button type="button" @click="chooseTextFile('source')">浏览…</button></div></div><div class="import-settings"><label>分段方式 <select v-model="sourceSegmentation" @change="previewFile('source')"><option value="non_empty_line">非空行</option><option value="sentence_rules">规则分句</option><option value="legacy_tagged_line">SISU 标记行</option></select></label><label>编码 <select v-model="sourceEncoding" @change="previewFile('source')"><option value="utf8">UTF-8</option><option value="utf8-bom">UTF-8 BOM</option><option value="gb18030">GB18030</option></select></label></div></section><section><div class="import-path"><label for="target-path">{{ displayLanguageLabel(targetLanguage) }}（译文）</label><div><FolderOpen :size="17" /><input id="target-path" v-model="targetPath" readonly placeholder="选择 TXT 译文" /><button type="button" @click="chooseTextFile('target')">浏览…</button></div></div><div class="import-settings"><label>分段方式 <select v-model="targetSegmentation" @change="previewFile('target')"><option value="non_empty_line">非空行</option><option value="sentence_rules">规则分句</option><option value="legacy_tagged_line">SISU 标记行</option></select></label><label>编码 <select v-model="targetEncoding" @change="previewFile('target')"><option value="utf8">UTF-8</option><option value="utf8-bom">UTF-8 BOM</option><option value="gb18030">GB18030</option></select></label></div></section></div><div class="preview-tabs"><button :class="{ active: previewTab === 'source' }" type="button" @click="previewTab = 'source'">{{ displayLanguageLabel(sourceLanguage) }}（原文） · {{ sourcePreview?.preview.segments.length ?? 0 }} 段</button><button :class="{ active: previewTab === 'target' }" type="button" @click="previewTab = 'target'">{{ displayLanguageLabel(targetLanguage) }}（译文） · {{ targetPreview?.preview.segments.length ?? 0 }} 段</button></div><div class="segment-preview"><div v-if="!activePreview" class="preview-empty">选择文本后会以指定编码和规则生成真实预览。</div><div v-for="text in activePreview?.preview.segments ?? []" :key="text.ordinal" class="preview-line"><span>{{ String(text.ordinal + 1).padStart(2, '0') }}</span><p>{{ text.content }}</p></div></div><footer><span class="import-contract">源文件只读；工程写入 <b>.jm</b> 文件夹并保留稳定 ID。</span><button class="secondary-button" type="button" @click="importOpen = false">取消</button><button class="primary-button" type="button" :disabled="busy" @click="createProjectFromPreview"><Check :size="15" />创建并打开工程</button></footer></section></div>
  </div>
</template>
...
.settings-view { justify-content: flex-start; max-width: 700px; overflow: auto; }.settings-card { display: grid; gap: 14px; width: min(630px, 76vw); padding: 20px; border: 1px solid var(--line); border-radius: 8px; color: var(--ink-700); background: var(--paper); }.settings-card h3 { margin: 4px 0 0; padding-bottom: 8px; border-bottom: 1px solid var(--line); color: var(--ink-900); font-size: 14px; }.settings-card h3:not(:first-child) { margin-top: 10px; }.settings-card label { display: grid; grid-template-columns: 115px 1fr auto; align-items: center; gap: 10px; font-size: 13px; }.settings-card select { height: 34px; padding: 0 9px; border: 1px solid #c8d3c9; border-radius: 5px; color: var(--ink-900); background: var(--paper); }.settings-card output { min-width: 42px; color: var(--green-900); }.settings-card > small { margin-top: -7px; padding-left: 125px; color: var(--ink-500); line-height: 1.5; }.settings-card code { padding: 1px 4px; border-radius: 3px; background: #f0f4f0; font-size: 10px; }.settings-card > span { display: flex; align-items: center; gap: 8px; padding-top: 13px; border-top: 1px solid var(--line); color: var(--green-700); font-size: 12px; }.toggle-setting input { justify-self: start; width: 17px; height: 17px; accent-color: var(--green-700); }.toggle-setting span { min-width: 48px; color: var(--green-900); font-size: 12px; }.shortcut-grid { display: grid; grid-template-columns: 1fr auto; gap: 0; margin-left: 125px; overflow: hidden; border: 1px solid var(--line); border-radius: 6px; background: #fafcfa; }.shortcut-grid > span, .shortcut-grid kbd { padding: 7px 10px; border-bottom: 1px solid var(--line); font-size: 11px; }.shortcut-grid > :nth-last-child(-n + 2) { border-bottom: 0; }.shortcut-grid kbd { min-width: 130px; border-left: 1px solid var(--line); color: var(--green-900); background: #fff; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; text-align: center; }.settings-card > .shortcut-note { margin-top: -5px; }.cache-actions { display: flex; align-items: center; gap: 12px; padding-left: 125px; }.cache-actions span { color: var(--ink-500); font-size: 11px; }.cache-actions button:disabled { opacity: .55; cursor: wait; }.bookmark-list { display: grid; gap: 8px; width: min(560px, 72vw); max-height: 420px; overflow: auto; }.bookmark-list article { display: grid; grid-template-columns: 1fr 36px; align-items: center; border: 1px solid var(--line); border-radius: 7px; background: var(--paper); }.bookmark-list button { display: flex; align-items: center; gap: 11px; min-width: 0; padding: 11px 13px; border: 0; color: var(--green-900); background: transparent; text-align: left; cursor: pointer; }.bookmark-list button:last-child { justify-content: center; padding-inline: 8px; color: var(--ink-500); }.bookmark-list span { display: grid; gap: 3px; }.bookmark-list small { color: var(--ink-500); font-family: ui-monospace, Consolas, monospace; }.bookmark-list em { overflow: hidden; color: var(--ink-700); font-size: 12px; font-style: normal; line-height: 1.4; text-overflow: ellipsis; white-space: nowrap; }.empty-copy { padding: 20px; border: 1px dashed #c8d3c9; border-radius: 7px; }.project-actions { display: flex; gap: 9px; }
