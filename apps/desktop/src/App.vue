...
import AlignmentProgress from "./components/AlignmentProgress.vue";
...
const workspaceScrollProgress = ref(0);
const scrollWorkspaceToProgress = (ratio: number) => (showComparison.value ? comparisonWorkspaceRef.value : parallelWorkspaceRef.value)?.scrollToProgress(ratio);
...
const comparisonWorkspaceRef = ref<{ scrollToProgress: (ratio: number) => void; openFind: () => Promise<void>; navigateFind: (direction: number) => Promise<void> } | null>(null);
...
let pendingAnnotationWidth: number | null = null;
const flushAnnotationResize = () => {
  if (annotationResizeFrame !== null) cancelAnimationFrame(annotationResizeFrame);
  annotationResizeFrame = null;
  if (pendingAnnotationWidth !== null) setAnnotationWidth(pendingAnnotationWidth);
  pendingAnnotationWidth = null;
};
const resizeAnnotationPanel = (event: PointerEvent) => {
  pendingAnnotationWidth = window.innerWidth - event.clientX;
  if (annotationResizeFrame === null) annotationResizeFrame = requestAnimationFrame(flushAnnotationResize);
};
const stopAnnotationResize = () => {
  if (!annotationResizing.value) return;
  flushAnnotationResize();
...
<template>
  <div class="app-shell" :class="{ 'app-shell--tutorial': tutorialActive }">
    ...
    <div class="content-grid" :class="{ 'content-grid--collapsed': sideNavCollapsed, 'content-grid--annotation': rightPanelOpen, 'content-grid--resizing': annotationResizing }" :style="{ '--annotation-width': `${annotationWidth}px` }">
      ...
      <main class="main-stage">
        ...
        <div v-if="activeNav === 'parallel'" class="parallel-stage">
          <ComparisonWorkspace @scroll-progress="workspaceScrollProgress = $event" ref="comparisonWorkspaceRef" :find-in-view="workspaceText.find" v-if="showComparison && projectSnapshot" :project="projectSnapshot" :texts="workspaceText.texts.value" :writable="workspaceWritable && !busy" @visible-segments="loadVisibleSegments" @edit-pair="openComparisonPair" @unlink="unlinkAlignment" />
    ...
    <footer class="bottom-status">
      ...
      <AlignmentProgress :scroll-progress="workspaceScrollProgress" :source="sourceRows" :target="targetRows" :alignments="alignmentRows" @navigate="scrollWorkspaceToProgress" />
    </footer>
...
.annotation-resize-handle:hover, .annotation-resize-handle:focus-visible, .content-grid--resizing .annotation-resize-handle { @apply bg-accent; box-shadow: 0 0 0 1px rgb(47 129 67 / 12%); }
.search-view { display: flex; flex-direction: column; height: 100%; min-height: 0; padding: 25px 28px; }.search-bar { display: flex; align-items: center; gap: 10px; }.search-bar > label { width: 42px; font-size: var(--jm-font-size-body); line-height: var(--jm-line-height-body); }.search-input-wrap { display: flex; align-items: center; width: min(570px, 48vw); height: 42px; padding: 0 12px; border: 1px solid #c9d1ca; border-radius: 7px 0 0 7px; @apply text-ink-500; }.search-input-wrap input { flex: 1; min-width: 0; padding: 0 9px; border: 0; outline: none; }.search-input-wrap button { padding: 3px; border: 0; background: transparent; @apply text-ink-500; cursor: pointer; }.primary-search, .primary-button { display: inline-flex; align-items: center; justify-content: center; gap: 6px; height: 42px; padding: 0 22px; border: 1px solid var(--green-900); border-radius: 0 6px 6px 0; color: #fff; @apply bg-accent-solid; cursor: pointer; }.filter-button { height: 42px; margin-left: -10px; padding: 0 12px; border: 1px solid #c9d1ca; border-radius: 0 7px 7px 0; @apply bg-raised; cursor: pointer; }.check-label { display: inline-flex; align-items: center; gap: 6px; margin-left: 22px; @apply text-ink-700; font-size: var(--jm-font-size-body); line-height: var(--jm-line-height-body); }.current-project { display: inline-flex; align-items: center; gap: 4px; margin-left: auto; @apply text-accent-strong; font-size: var(--jm-font-size-body); line-height: var(--jm-line-height-body); }.search-summary { padding: 15px 5px 12px; @apply text-ink-700; font-size: var(--jm-font-size-body); line-height: var(--jm-line-height-body); }.search-summary strong { @apply text-accent-strong; }.search-summary span { @apply text-ink-500; }.search-table { min-height: 0; overflow: auto; border: 1px solid var(--line); border-radius: 7px; }.search-table-head, .search-row { display: grid; grid-template-columns: 85px 1.2fr 95px 1.45fr 90px; align-items: center; gap: 14px; padding: 0 17px; }.search-table-head { height: 43px; @apply text-ink-700 bg-raised; font-size: var(--jm-font-size-callout); line-height: var(--jm-line-height-callout); }.search-row { width: 100%; min-height: 57px; border: 0; border-top: 1px solid var(--line); @apply bg-raised text-ink-900; font-size: var(--jm-font-size-body); text-align: left; cursor: pointer; line-height: var(--jm-line-height-body); }.search-row:hover { @apply bg-green-soft; }.search-row > span { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.search-row mark { width: max-content; padding: 2px 7px; color: var(--text-warning, #72591b); @apply bg-warm-soft; font-weight: var(--jm-font-weight-semibold); }
.aux-view { display: flex; flex-direction: column; align-items: flex-start; justify-content: center; gap: 13px; max-width: 540px; height: 100%; margin: auto; padding: 32px; @apply text-accent-strong; }.aux-view h2 { margin: 0; @apply text-ink-900; font-size: var(--jm-font-size-title-2); line-height: var(--jm-line-height-title-2); font-weight: var(--jm-font-weight-regular); }.aux-view p { margin: 0; @apply text-ink-700; line-height: 1.6; }.secondary-button { display: inline-flex; align-items: center; justify-content: center; gap: 6px; height: 38px; padding: 0 17px; border: 1px solid #bdcabf; border-radius: 6px; @apply text-ink-700 bg-raised; cursor: pointer; }.aux-view .primary-button { border-radius: 6px; }.project-summary-card { display: grid; grid-template-columns: auto auto; gap: 3px 14px; margin: 4px 0; padding: 14px 18px; border: 1px solid var(--line); border-radius: 8px; @apply text-ink-900; }.project-summary-card span { font-size: var(--jm-font-size-title-1); @apply text-accent-strong; line-height: var(--jm-line-height-title-1); }.project-summary-card small { @apply text-ink-500; font-size: var(--jm-font-size-subheadline); line-height: var(--jm-line-height-subheadline); }
.bookmark-list { display: grid; gap: 8px; width: min(560px, 72vw); max-height: 420px; overflow: auto; }.bookmark-list article { display: grid; grid-template-columns: 1fr 36px; align-items: center; border: 1px solid var(--line); border-radius: 7px; @apply bg-paper; }.bookmark-list button { display: flex; align-items: center; gap: 11px; min-width: 0; padding: 11px 13px; border: 0; @apply text-accent-strong; background: transparent; text-align: left; cursor: pointer; }.bookmark-list button:last-child { justify-content: center; padding-inline: 8px; @apply text-ink-500; }.bookmark-list span { display: grid; gap: 3px; }.bookmark-list small { @apply text-ink-500; font-family: var(--jm-font-mono); }.bookmark-list em { overflow: hidden; @apply text-ink-700; font-size: var(--jm-font-size-callout); font-style: normal; line-height: 1.4; text-overflow: ellipsis; white-space: nowrap; }.empty-copy { padding: 20px; border: 1px dashed #c8d3c9; border-radius: 7px; }.project-actions { display: flex; align-items: center; gap: 9px; }.project-actions > button { height: 42px; }
.modal-backdrop { position: fixed; z-index: 20; inset: 0; display: grid; place-items: center; background: rgb(31 42 34 / 22%); }.eyebrow { @apply text-accent; font-size: var(--jm-font-size-subheadline); font-weight: var(--jm-font-weight-semibold); letter-spacing: .1em; line-height: var(--jm-line-height-subheadline); }
.annotation-panel { position: fixed; z-index: 10; top: 116px; right: 0; bottom: 58px; display: flex; flex-direction: column; width: 355px; overflow: auto; border-left: 1px solid #ccd9cd; @apply bg-raised; box-shadow: -12px 0 30px rgb(32 51 35 / 10%); }.annotation-panel header { display: flex; align-items: flex-start; justify-content: space-between; padding: 19px 18px 12px; }.annotation-panel header button { padding: 5px; border: 0; background: transparent; @apply text-ink-500; cursor: pointer; }.annotation-panel h2 { margin: 4px 0 0; @apply text-ink-900; font-size: var(--jm-font-size-title-3); line-height: var(--jm-line-height-title-3); font-weight: var(--jm-font-weight-semibold); }.annotation-panel h2 small { display: inline-block; margin-left: 3px; padding: 2px 6px; border-radius: 10px; @apply text-accent-strong bg-accent-soft; font-size: var(--jm-font-size-subheadline); line-height: var(--jm-line-height-subheadline); }.annotation-filter { display: flex; gap: 4px; padding: 0 13px 13px; border-bottom: 1px solid var(--line); }.annotation-filter button { padding: 7px 8px; border: 1px solid transparent; border-radius: 5px; @apply text-ink-500; background: transparent; font-size: var(--jm-font-size-callout); cursor: pointer; line-height: var(--jm-line-height-callout); }.annotation-filter button.active { border-color: #b3d5b7; @apply text-accent-strong bg-green-soft; }.annotation-card { display: grid; grid-template-columns: 25px 1fr; gap: 7px; margin: 13px 13px 0; padding: 13px 11px; border: 1px solid #d3dde5; border-radius: 8px; @apply bg-raised; }.annotation-card--draft { border-color: #d3c4ec; }.annotation-number { display: grid; place-items: center; width: 22px; height: 22px; border-radius: 6px; color: #fff; background: #9864d5; font-size: var(--jm-font-size-callout); line-height: var(--jm-line-height-callout); }.annotation-number--green { background: #3c9a5a; }.annotation-state { display: flex; align-items: center; gap: 7px; @apply text-ink-900; font-size: var(--jm-font-size-callout); font-weight: var(--jm-font-weight-semibold); line-height: var(--jm-line-height-callout); }.annotation-state span { @apply text-ink-500; font-weight: var(--jm-font-weight-medium); }.annotation-state time { margin-left: auto; @apply text-ink-500; font-size: var(--jm-font-size-subheadline); font-weight: var(--jm-font-weight-regular); line-height: var(--jm-line-height-subheadline); }.annotation-card h3 { margin: 12px 0 7px; @apply text-ink-900; font-size: var(--jm-font-size-body); line-height: var(--jm-line-height-body); font-weight: var(--jm-font-weight-semibold); }.annotation-card p { margin: 0; @apply text-ink-700; font-size: var(--jm-font-size-body); line-height: 1.55; }.annotation-links { display: flex; flex-direction: column; gap: 5px; margin-top: 10px; padding: 8px; border-radius: 5px; @apply bg-raised text-ink-700; font-size: var(--jm-font-size-subheadline); line-height: var(--jm-line-height-subheadline); }.annotation-links b { float: right; @apply text-ink-500; font-weight: var(--jm-font-weight-medium); }.annotation-card footer { display: flex; gap: 15px; margin-top: 10px; padding-top: 9px; border-top: 1px solid var(--line); }.annotation-card footer button, .resolve-button, .new-annotation { display: inline-flex; align-items: center; gap: 5px; border: 0; @apply text-ink-700; background: transparent; font-size: var(--jm-font-size-callout); cursor: pointer; line-height: var(--jm-line-height-callout); }.resolve-button { margin-top: 11px; padding: 6px 9px; border: 1px solid #b5dab9; border-radius: 5px; @apply text-accent-strong bg-green-soft; }.new-annotation { justify-content: center; margin: 13px; padding: 10px; border: 1px solid #abd1af; border-radius: 6px; @apply text-accent-strong bg-green-soft; }
.mode-guard-backdrop { z-index: 30; }.mode-guard { width: min(460px, calc(100vw - 60px)); padding: 24px; border: 1px solid #d6c58f; border-radius: 10px; @apply bg-raised; box-shadow: 0 22px 70px rgb(29 48 32 / 23%); }.mode-guard h2 { margin: 5px 0 9px; @apply text-ink-900; font-size: var(--jm-font-size-title-2); line-height: var(--jm-line-height-title-2); font-weight: var(--jm-font-weight-regular); }.mode-guard p { margin: 0; @apply text-ink-700; font-size: var(--jm-font-size-body); line-height: 1.6; }.mode-guard > div { display: flex; justify-content: flex-end; gap: 9px; margin-top: 20px; }.mode-guard .primary-button { height: 38px; border-radius: 6px; }.danger-button { border-color: #dab7b7; color: var(--text-danger, #9b4e4e); }
</style>

<style scoped>
.parallel-stage > .workspace { flex: 1; height: auto; }
.parallel-stage { display: flex; flex-direction: column; height: 100%; min-height: 0; }
.comparison-return { display: flex; align-items: center; gap: 12px; padding: 8px 16px; border-bottom: 1px solid var(--line); @apply bg-subtle; }
