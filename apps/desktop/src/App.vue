...
const applySnapshot = async (snapshot: ProjectSnapshot, resetView = false) => {
  ...
  dirty.value = false;
  if (resetView) { activeNav.value = "parallel"; activeMode.value = "review"; }
};
onMounted(async () => {
...
const setMode = (mode: WorkspaceMode) => { if (mode === "order" && activeMode.value !== "order") orderBaseline.value = sourceRows.value.map((segment) => segment.id); activeMode.value = mode; activeNav.value = mode === "history" ? "history" : "parallel"; exportOpen.value = false; if (mode === "history") void selectHistoryRevision(previousRevisionId()); };
const setNav = (nav: NavId) => { activeNav.value = nav; if (nav === "annotations") annotationOpen.value = true; if (nav === "history") { activeMode.value = "history"; void selectHistoryRevision(previousRevisionId()); } else if (nav === "parallel" && activeMode.value === "history") activeMode.value = "review"; };
...
const comparisonToDiff = (comparison: RevisionComparison): HistoryDiff => {
  ...
  return {
    segmentId: (sourceChange ?? targetChange)?.segment_id.slice(0, 8) ?? "结构变更",
...
const runSearch = async (options: SearchQueryOptions) => {
  ...
  try {
    ...
    searchResults.value = response.hits.map((hit) => {
      ...
      return { id: hit.segment_id, sourceId, targetId, sourceText: sourceId ? sourceById.get(sourceId) ?? "" : "", targetText: targetId ? targetById.get(targetId) ?? "" : "", alignmentId: hit.alignment_id };
    });
...
const selectSearchResult = (result: SearchResult) => { selectedAlignmentId.value = result.alignmentId ?? ""; activeNav.value = "parallel"; activeMode.value = "review"; };
const toggleBookmark = async (segmentId: string, alignmentId: string) => {
  ...
  try { await applySnapshot(existing ? await kernelClient.deleteBookmark(existing.bookmark_id) : await kernelClient.createBookmark(segmentId, alignmentId, `书签 ${segmentId.slice(0, 8)}`)); notify(existing ? "已移除书签" : "已添加书签"); }
  catch (error) { notify(`书签操作失败：${errorMessage(error)}`); }
};
const openBookmark = (alignmentId: string | null) => { if (!alignmentId) return; selectedAlignmentId.value = alignmentId; activeNav.value = "parallel"; activeMode.value = "review"; };
...
<template>
  <div class="app-shell">
    ...
    <div class="content-grid" :class="{ 'content-grid--collapsed': sideNavCollapsed }">
