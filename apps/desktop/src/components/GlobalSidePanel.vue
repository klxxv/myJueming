...
<template>
  <section class="global-panel" aria-label="全局助手与批注" data-agent-context="exclude">
    <header class="global-panel__header">
      <div class="global-panel__tabs" role="tablist" aria-label="右侧栏内容" @keydown="navigateTabs">
        <button id="global-agent-tab" type="button" role="tab" :tabindex="activeTab === 'agent' ? 0 : -1" :aria-selected="activeTab === 'agent'" aria-controls="global-agent-content" @click="activeTab = 'agent'"><Sparkles :size="16" />助手<span v-if="pendingCount" class="count">{{ pendingCount }}</span></button>
        <button id="global-annotations-tab" type="button" role="tab" :tabindex="activeTab === 'annotations' ? 0 : -1" :aria-selected="activeTab === 'annotations'" aria-controls="global-annotations-content" @click="activeTab = 'annotations'"><MessageSquareText :size="16" />批注<span class="count">{{ annotationCount }}</span></button>
      </div>
      <button class="global-panel__close" type="button" aria-label="收起右侧栏" @click="emit('close')"><X :size="17" /></button>
    </header>
...
.global-panel__content { flex: 1; min-height: 0; overflow: hidden; }
