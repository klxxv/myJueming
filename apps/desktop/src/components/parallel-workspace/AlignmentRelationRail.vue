...
.alignment-relation__pill { position: absolute; z-index: 3; top: var(--connector-y); left: 50%; display: inline-flex; min-width: 0; overflow: hidden; border: 1px solid #a6cdaa; border-radius: 999px; @apply text-accent-strong bg-green-soft; box-shadow: 0 1px 3px rgb(31 53 34 / 8%); transform: translate(-50%, -50%); }
...
.alignment-relation__label:hover, .alignment-relation__quick:hover:not(:disabled) { @apply bg-green-selected; }
.alignment-relation--active .alignment-relation__pill { @apply border-accent; box-shadow: 0 0 0 2px rgb(47 129 67 / 15%); }
.alignment-relation--selected .alignment-relation__pill { color: #fff; @apply bg-accent-solid; }
.alignment-relation--selected .alignment-relation__quick { border-left-color: rgb(255 255 255 / 34%); }
.alignment-relation:not(.alignment-relation--linked) .alignment-relation__pill { border-color: #e5b74d; color: var(--text-warning, #a66f00); @apply bg-warm-soft; }
