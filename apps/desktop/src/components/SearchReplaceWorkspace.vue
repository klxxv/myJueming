...
// Measure wrapped rows again when ResizeObserver detects content or width changes.
const measureResultRow = (element: Element | ComponentPublicInstance | null) => {
  if (element instanceof HTMLButtonElement) resultVirtualizer.value.measureElement(element);
};
  ...
  <section class="search-replace-workspace" :aria-label="t('rwSearchReplace')">
    ...
    <div ref="resultViewport" class="sr-table">
      ...
      <div v-if="matchingResults.length" class="sr-virtual-body" :style="{ height: `${resultVirtualizer.getTotalSize()}px` }">
        <button v-for="{ result, virtualRow } in virtualResults" :key="result.id" :ref="measureResultRow" :data-index="virtualRow.index" class="sr-row" type="button" :style="{ transform: `translateY(${virtualRow.start}px)` }" @click="selectResult($event, result)">
...
.sr-table-head, .sr-row { box-sizing: border-box; min-width: 600px; grid-template-columns: 62px minmax(150px, 1fr) 74px minmax(150px, 1.2fr) 70px; gap: 10px; padding-inline: 12px; }
