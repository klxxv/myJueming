...
export const useOrderDragAndDrop = ({
  ...
  onReorder: (side: LanguageSide, orderedSegmentIds: string[]) => void;
  ...
  const instanceId = Symbol("jueming-order-lists");
  ...
  // Match the official PDD list pattern: the handle is the native draggable,
  // while the full Segment root is the drop target. They are registered and
  // cleaned up together by the Segment component.
  ...
  const cleanupMonitor = monitorForElements({
    canMonitor: ({ source }) => isSegmentDrag(source.data) && source.data.instanceId === instanceId,
    onDrop: ({ location, source }) => {
      const target = location.current.dropTargets[0];
      if (!target || target.data.instanceId !== instanceId || target.data.side !== source.data.side) return;
